document.addEventListener('DOMContentLoaded', () => {
  ...
  function setLoading(loading) {
    ...
    if (buttonText) buttonText.textContent = loading ? t('retrieving') : t('getVideo');
  }
  ...
  function isTikTokUrl(value) {
    try {
      ...
      const allowedHosts = ['tiktok.com','vm.tiktok.com','vt.tiktok.com','m.tiktok.com','douyin.com','www.douyin.com'];
  ...
  async function readJson(response) {
    ...
    if (!contentType.includes('application/json')) throw new Error(`${t('unexpected')} (${response.status}).`);
    let data;
    try { data = await response.json(); } catch { throw new Error(t('invalidJson')); }
    if (!response.ok) throw new Error(data?.error?.message || `${t('requestFailed')} (${response.status}).`);
    return data;
  }
  ...
  pasteBtn.addEventListener('click', async () => {
    ...
    try {
      ...
      if (!text) { showError(t('clipboardEmpty')); return; }
      urlInput.value = text.trim();
      urlInput.focus();
    } catch (error) { showError(error?.message || t('clipboardManual')); }
  });

  clearBtn.addEventListener('click', () => {
    urlInput.value = '';
    hideError(); hideStatus(); resetResult(); urlInput.focus();
  });
  ...
  form.addEventListener('submit', async event => {
    ...
    try {
      ...
      if (typeof data.videoDuration === 'number' && data.videoDuration > 0) {
        const totalSeconds = Math.round(data.videoDuration);
        const minutes = Math.floor(totalSeconds / 60);
        const seconds = String(totalSeconds % 60).padStart(2, '0');
        resDuration.textContent = `${minutes}:${seconds}`;
      } else resDuration.textContent = '';
      ...
      if (data.hdDownloadUrl) {
        downloadHd.href = data.hdDownloadUrl;
        downloadHd.classList.remove('hidden');
        downloadHd.setAttribute('aria-label', t('downloadHd'));
      } else {
        downloadHd.removeAttribute('href');
        downloadHd.classList.add('hidden');
      }
