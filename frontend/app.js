document.addEventListener('DOMContentLoaded', () => {
  ...
  function applyLanguage(){const isAr=lang==='ar';document.documentElement.lang=lang;document.documentElement.dir=isAr?'rtl':'ltr';document.body.dir=isAr?'rtl':'ltr';languageBtn.textContent=isAr?'English':'العربية';languageBtn.setAttribute('aria-label',isAr?'Switch to English':'التبديل إلى العربية');urlInput.placeholder=t('placeholder');urlInput.setAttribute('aria-label',isAr?'رابط فيديو تيك توك':'TikTok video URL');document.querySelectorAll('[data-i18n]').forEach(el=>{el.textContent=t(el.dataset.i18n)});document.title=isAr?'تحميل فيديو تيك توك بدون علامة مائية | TikVideo':'TikTok Video Downloader Without Watermark | TikVideo';const desc=document.querySelector('meta[name="description"]');if(desc)desc.content=isAr?'حمّل فيديوهات تيك توك أونلاين بدون علامة مائية. الصق رابط فيديو TikTok العام واحصل على ملف MP4 المتاح بسرعة، بدون تسجيل أو تثبيت تطبيق.':'Download TikTok videos online without a watermark. Paste a public TikTok video URL and get the available MP4 quickly, with no registration or app installation.';if(!resultSection.classList.contains('hidden')){downloadStd.setAttribute('aria-label',t('downloadMp4'));if(!downloadHd.classList.contains('hidden'))downloadHd.setAttribute('aria-label',t('downloadHd'))}}
  ...
  function renderPhotoResult(data){
    ...
    const downloads=Array.isArray(data.imageDownloadUrls)?data.imageDownloadUrls:[];
    if(!images.length||!downloads.length)throw new Error(t('noDownload'));
    mediaCard?.classList.add('photo-result');
    resThumbnail.src=images[0];
    resThumbnail.classList.remove('hidden');
    resThumbnail.alt=lang==='ar'?'صورة TikTok':'TikTok photo';
    resDuration.textContent='';
    ...
    downloadStd.setAttribute('aria-label',t('downloadImage'));
    downloadHd.classList.add('hidden');
    ...
    if(photoGallery){
      ...
      for(let i=1;i<images.length;i++){
        ...
        link.append(image,index);
        photoGallery.append(link);
      }
      photoGallery.classList.toggle('hidden',images.length<2);
    }
  }
